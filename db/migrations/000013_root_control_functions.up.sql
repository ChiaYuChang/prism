...
BEGIN
    ...
    RETURNING tokens.id, tokens.type, tokens.name, tokens.permissions,
        tokens.hash_algorithm, tokens.created_at, tokens.expires_at,
        tokens.last_used_at, tokens.renewed_at, tokens.rotated_at, tokens.revoked_at;
END
$$;

CREATE OR REPLACE FUNCTION prism_root_check(
    p_hash_algorithm TEXT,
    p_token_hash TEXT
) RETURNS BOOLEAN
LANGUAGE sql
...
AS $$
    SELECT EXISTS (
        ...
        WHERE type = 'root'
          ...
          AND expires_at > NOW()
          AND hash_algorithm = p_hash_algorithm
          AND token_hash = p_token_hash
    )
$$;

CREATE OR REPLACE FUNCTION prism_root_create_admin(
    p_root_hash_algorithm TEXT,
    p_root_token_hash TEXT,
    p_id UUID,
    p_name TEXT,
    p_hash_algorithm TEXT,
    p_token_hash TEXT,
...
CREATE OR REPLACE FUNCTION prism_root_revoke_all(
    p_hash_algorithm TEXT,
    p_token_hash TEXT
) RETURNS BIGINT
LANGUAGE plpgsql
