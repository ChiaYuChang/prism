...
* **`batches` is a system-internal completion unit; `user_fetch_requests` is a user-facing observation unit. They must not be conflated.** The two layers exist for different purposes and have different correctness requirements.
  ...
  * **`POST /page_fetch` response shape — candidate-keyed items, three statuses only.** Response body is `{request_id, items: [{candidate_id, status}]}` with `status ∈ {created, already_complete, not_found}`. The TUI / UI work needs candidate-keyed status so each row in the source list can be marked individually; aggregate counts alone are too coarse. **`created` collapses fresh-insert and shared-active-task cases** — a request whose candidate maps onto another request's already-active task still receives `created`. Never expose `already_active` (leaks other-user / other-request activity) or `task_id` (cross-request shared identity). `not_found` items are echoed in the response only and are **not** persisted into `user_fetch_request_items` (no candidate row to FK against; would either require dropping the FK or inserting a sentinel). Aggregation in `GET /fetches/{request_id}` counts only stored items, so `not_found` never appears in progress totals.
