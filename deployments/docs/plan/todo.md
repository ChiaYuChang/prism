...
* [ ] 4.3 Laptop/Home-Server Deployment:
  * [x] **Cron-triggered DIRECTORY_FETCH:** add the `schedules` table, config sync from `configs/trigger/cron/schedules.yaml`, and the `cmd/trigger/cron` heartbeat materializer. The trigger claims due schedules with `FOR UPDATE SKIP LOCKED`, creates/recover tasks and advances `next_fire_at` in one transaction, then leaves dispatch to the existing `cmd/scheduler` → Watermill → worker path.
