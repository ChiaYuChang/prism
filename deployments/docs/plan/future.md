...
* [ ] **Adopt testcontainers-go for integration tests + GitHub Actions CI.**
  * **Why:** `internal/collector/archiver/s3_test.go` previously required SeaweedFS at `localhost:8333` (via `task compose:infra`). `services:` in GH Actions would fix CI but diverges from local and gives one shared instance per job rather than per-test isolation.
