...
10. Candidate and content embeddings are generated by the configured analyzer
    pipeline after a completed collection batch is promoted to a pipeline Root.
...
PAGE_FETCH rate limiting is currently process-local and keyed by `source_abbr`.
A Valkey-backed limiter is deferred until scheduler execution becomes
short-lived or horizontally scaled.
...
Completed COLLECTION batch
 └─► admin pipeline request
      └─► Root batch + PIPELINE_INIT task in one transaction
           └─► configured stages from configs/llm_pipeline.yaml
                ├─► EMBED_CANDIDATE tasks over Root snapshots
                └─► EMBED_CONTENT tasks over Root snapshots
```

The pipeline definition is deployed configuration, not a request parameter.
Root creation takes a `REPEATABLE READ` snapshot of complete candidates and
readable contents, atomically with Root/P0 creation. Stage work reads those
Root-keyed snapshots rather than mutable source rows.
...
## 4. Analysis And Pre-Analysis Boundaries

The current analyzer runtime accepts a completed `COLLECTION` batch through an
admin endpoint and creates a pipeline Root plus `PIPELINE_INIT` task. The
configured pipeline is fixed to `configs/llm_pipeline.yaml`; idempotency is
scoped to the input batch and definition hash. See `docs/pipeline.md` for its
operational contract.

The planned user-facing path is separate and must precede that Root creation:
...
* **Users currently interact with the system by querying `candidates` and requesting PAGE_FETCH.** The planned analysis-run flow adds preflight, acquisition progress, and a confirmed manifest before user-selected inputs reach analyzer Root/P0 creation.
* **PAGE_FETCH has two distinct triggers:** PARTY press releases are fetched automatically by the Discovery Worker after candidate persistence; MEDIA articles are fetched on explicit user selection. Both go through the `tasks` table and `scheduler-fast`.
...
* Two scheduler instances share one binary: `scheduler-slow` handles `DIRECTORY_FETCH`, `KEYWORD_SEARCH`, and analyzer work; `scheduler-fast` handles `PAGE_FETCH` (3s poll). PAGE_FETCH rate limiting is currently in-memory; the Valkey migration is deferred in `future.md`.
...
* **`internal/collector/saver/` has been replaced by `internal/collector/archiver/`.** `LocalArchiver` and `S3Archiver` are selected by URI scheme: `file:///path` for local, `s3://bucket/prefix` for S3.
...
* **LLM parser has two opt-in roles, never automatic promotion.** (a) **Parse-time fallback extractor:** a global `fallback.enable: true` in `parsers.yaml` activates an LLM-backed `collector.Parser` that handles any host without a registered entry. The Registry routes host-misses to the fallback parser when configured; otherwise it returns `ErrNoMatchingParser` (existing behavior). The provider, model, and key path live alongside the toggle in the same yaml (`fallback.llm.{provider,model,key_file,timeout}`) so the operator sees the full activation surface in one place — no hidden flag/env state. The API key arrives via `key_file` (path to a mounted secret), mirroring `PostgresConfig.PasswordFile` / `ValkeyConfig.PasswordFile`; inline `key:` is supported for local dev but discouraged for production. (b) **Config-snippet generator for human review:** `LLMArticleContent.ToConfigSnippet(host)` produces a YAML fragment suitable for pasting under `parsers:` after an engineer verifies selector stability across multiple pages. Both roles are explicitly opt-in. There is no automatic promotion: the operator workflow for adopting a new site is *enable global fallback → site is parseable via LLM → operator reviews extracted samples + LLM-generated snippet → if quality acceptable, operator adds a real `html`/`jsonld` rule for that host → fallback no longer fires for it*. v1 assumes the input bytes reaching the parser are HTML-shape; per-input-type fallback (json/xml/rss with custom prompts) is a follow-up. Per-host fallback overrides are deliberately not supported — fallback is a global toggle to keep the activation surface small and reviewable.
...
* **`batches` is a system-internal completion unit; `fetches` is a user-facing observation unit. They must not be conflated.** The two layers exist for different purposes and have different correctness requirements. (Tables are named `fetches` / `fetch_items` without a `user_` prefix so a future migration can move them into a dedicated `user` schema — `user.fetches`, `user.fetch_items`, `user.users` — via `ALTER TABLE ... SET SCHEMA "user"` without renaming.)
  ...
  * **`fetches` (new for Phase 2.7):** groups the candidates a user selected in one `POST /page_fetch` call. Completion gates user-facing notification, not system work. Each `fetch_items` row snapshots which underlying `tasks.id` is doing the actual work. Multiple fetches (across users / across queries) can target the same URL and therefore share the same active task — every fetch observes its items terminal when that task terminates, and no fetch sees another fetch's existence. This is required for multi-user / multi-query scenarios where one user must not learn that another user is interested in the same article (side-channel privacy).
  * **Why two tables and not one:** discovery batch completion semantics (`count(candidates) > 0 AND count(candidates) <= count(contents)`) do not apply to user fetches (which have no candidates of their own — they reference existing ones). Fetch completion is `all items in terminal state`. Forcing both into `batches` requires a `kind` discriminator + branched completion query + extra columns the discovery side does not need. Cleaner to keep `batches` semantically pure and add a parallel table for the user-facing layer.
  ...
  * **`CreateTask` must return the existing active task on conflict.** Today it returns `ErrTaskAlreadyActive` only — the user-fetch handler needs the task_id to record `fetch_items.task_id`. Either (a) extend the SQLC query to `INSERT … ON CONFLICT … RETURNING id` (rows from both insert and conflict paths) or (b) add `GetActivePageFetchTaskByURL(url)` and call it after the conflict. (a) is one round-trip, (b) is two — prefer (a).
  * **`fetch_items.task_id` is nullable; `fetch_items.snapshot_status` is nullable.** When `CreateTask` conflict + lookup both miss (task already terminal between conflict and lookup), check `contents` by URL: if present, insert item with `snapshot_status='ALREADY_COMPLETE'` and `task_id=NULL`. Live items have `snapshot_status=NULL` and resolve status by joining `tasks`. Aggregator: `COALESCE(snapshot_status, tasks.status)`.
  * **Cross-user privacy.** `GET /fetches/{id}` is filtered by `fetch_id` only (and, when authn lands, by `user_id`). Aggregation never returns task_ids or other-fetch membership. The fact that an item points at a shared task is not surfaced. Skipped/duplicate handling stays internal.
  * **`POST /page_fetch` response shape — candidate-keyed items, three statuses only.** Response body is `{fetch_id, items: [{candidate_id, status}]}` with `status ∈ {created, already_complete, not_found}`. The TUI / UI work needs candidate-keyed status so each row in the source list can be marked individually; aggregate counts alone are too coarse. **`created` collapses fresh-insert and shared-active-task cases** — a candidate that maps onto another fetch's already-active task still receives `created`. Never expose `already_active` (leaks other-user / other-fetch activity) or `task_id` (cross-fetch shared identity). `not_found` items are echoed in the response only and are **not** persisted into `fetch_items` (no candidate row to FK against; would either require dropping the FK or inserting a sentinel). Aggregation in `GET /fetches/{id}` counts only stored items, so `not_found` never appears in progress totals.
  * **Notification (deferred):** `fetches.completed_at` is set by a background sweeper or computed on-the-fly; v1 computes on-the-fly to avoid a new sweeper job. When notification (email/webhook) is wired in v2, persist `completed_at` so transition is detectable.
