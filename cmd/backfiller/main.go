...
func parseCLI(args []string, output io.Writer) (cliOptions, error) {
	...
	fs.StringVar(&opts.natsCfg.Host, "nats-host", "localhost", "The NATS server host")
	fs.IntVar(&opts.natsCfg.Port, "nats-port", 4222, "The NATS server port")
	fs.StringVar(&opts.natsCfg.Token, "nats-token", "", "The NATS server auth token")
